...
_2fastmath::_2fastmath(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::_2fastmath)
{
    ui->setupUi(this);
    srand(time(nullptr));
...
_2fastmath::~_2fastmath()
{
    delete ui;
}

void _2fastmath::generateMathQuestion()
{
    if (numQuestions >= 15) {
        timer->stop();
    ...
    for (int i = 0; i < 3; ++i) {
        int randomNum = rand() % 10; // Generate random incorrect options
...
int _2fastmath::calculateResult(int num1, int num2, int operation)
{
...
void _2fastmath::on_pushButton_clicked()
{
    hide();
    fastmath window;
